// ═══════════════════════════════════════════════════════════════════════════
// MOONSHOT €20 — PumpFun bot v4
// Fix: usa DexScreener API (no bloquea) en vez de pump.fun API directa
// ═══════════════════════════════════════════════════════════════════════════
...
const SCAN_INTERVAL  = 45000; // 45s entre scans
...
// Filtros
const MIN_LIQUIDITY_USD  = 5000;  // mínimo $5000 de liquidez
const MIN_AGE_MINUTES    = 3;
const MAX_AGE_MINUTES    = 30;
const MIN_VOLUME_1H_USD  = 10000; // mínimo $10k volumen en 1h
const MAX_MCAP_LIQ_RATIO = 10;
const MIN_PRICE_CHANGE   = 5;     // mínimo +5% en últimos 5 min (momentum)
const MAX_PRICE_CHANGE   = 200;   // máximo +200% (si ya subió mucho, tarde)
...
function filterToken(pair) {
  ...
  // Debe ser en Solana y venir de PumpFun
  if (pair.chainId !== 'solana') fails.push('no-solana');
  if (!pair.dexId?.includes('pump') && !pair.labels?.includes('pump'))
    fails.push('no-pumpfun');

  // Edad del token
  const ageMin = pair.pairAge ? pair.pairAge / 60000 : 999;
  ...
  if (liq < MIN_LIQUIDITY_USD) fails.push(`liq:$${Math.round(liq)}<$${MIN_LIQUIDITY_USD}`);

  // Volumen en 1h
  const vol1h = pair.volume?.h1 || 0;
  if (vol1h < MIN_VOLUME_1H_USD) fails.push(`vol1h:$${Math.round(vol1h)}<$${MIN_VOLUME_1H_USD}`);
  ...
  // Momentum — precio subiendo en últimos 5 min
  const change5m = pair.priceChange?.m5 || 0;
  if (change5m < MIN_PRICE_CHANGE) fails.push(`momentum:${change5m.toFixed(1)}%<${MIN_PRICE_CHANGE}%`);
  if (change5m > MAX_PRICE_CHANGE) fails.push(`ya-subió:${change5m.toFixed(0)}%`);
...
// ── SCAN con DexScreener (no bloquea) ─────────────────────────────────────
async function scanRecentTokens() {
  ...
  try {
    // DexScreener: tokens nuevos en Solana ordenados por fecha
    ...
    for (const pair of pairs) {
      if (seenTokens.has(pair.tokenAddress || pair.pairAddress)) continue;
      const check = filterToken(pair);
      if (check.ok) {
        candidates.push({ pair, check });
      } else {
        totalSkips++;
    ...
    // Ordenar por score y comprar mejores
    candidates.sort((a, b) => b.check.score - a.check.score);
    console.log(`[SCAN] ${candidates.length} candidatos válidos de ${pairs.length} tokens`);

    for (const { pair, check } of candidates) {
      ...
      console.log(`[BUY ★${check.score}] ${sym} · ${check.ageMin.toFixed(1)}min · liq:$${Math.round(check.liq)} · +${check.change5m.toFixed(1)}%5m`);

      const result = await trade('buy', mint, SOL_PER_TRADE);
      if (result.success) {
        positions.set(mint, {
          mint, symbol: sym,
          entryPrice: parseFloat(pair.priceUsd || 0),
          amountSol: SOL_PER_TRADE,
          remainingSol: SOL_PER_TRADE,
          peakPrice: parseFloat(pair.priceUsd || 0),
          score: check.score,
        ...
        await notify(
          ...
          `📈 +${check.change5m.toFixed(1)}% en 5min · ratio:${check.ratio.toFixed(1)}x\n` +
          `[Ver en DexScreener](https://dexscreener.com/solana/${mint})`
        );
      } else {
...
async function monitorPositions() {
  ...
  for (const [mint, pos] of positions) {
    try {
      // Usa DexScreener para precio también
      const { data } = await axios.get(
...
app.listen(PORT, async () => {
  console.log(`🚀 MoonShot €20 Bot v4 — DexScreener API — puerto ${PORT}`);
  if (wallet) console.log(`👛 Wallet: ${wallet.publicKey.toBase58()}`);
  else console.log('⚠️  Sin WALLET_PRIVATE_KEY');
  await notify(
    '🚀 *Bot v4 activado — DexScreener*\n' +
    '✅ Fix: nueva API sin bloqueos\n' +
    `📊 Filtros: ${MIN_AGE_MINUTES}-${MAX_AGE_MINUTES}min · liq>$${MIN_LIQUIDITY_USD} · vol1h>$${MIN_VOLUME_1H_USD} · momentum>+${MIN_PRICE_CHANGE}%`
  );
  scanRecentTokens();
});
